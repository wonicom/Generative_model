...
def random_warp(image):
    ...
    mapx = mapx + np.random.normal(size=(5, 5), scale=5*h/150)
    mapy = mapy + np.random.normal(size=(5, 5), scale=5*h/150)
...
def train(data_path: str, model_name='Quick96', new_model=False, saved_models_dir='saved_model'):
    ...
    if not new_model and (saved_models_dir / f'{model_name}.pth').exists():
        ...
        saved_model = torch.load(str(saved_models_dir / f'{model_name}.pth'),map_location=torch.device('cpu'))
