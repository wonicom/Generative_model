...
def extract_and_align_face_from_image(input_dir: Union[str, Path], desired_face_width: int = 150) -> None:
    """
...
class FaceExtractor:
    ...
    @staticmethod
    def align(image, face, desired_face_width=150, left_eye_desired_coordinate=np.array((0.37, 0.37))):
        """
    ...
    @staticmethod
    def extract(image, face, desired_face_width=150, left_eye_desired_coordinate=np.array((0.37, 0.37))):
        """
        Align the face so the eyes will be at the same level
        Args:
            image (np.ndarray): image with face
